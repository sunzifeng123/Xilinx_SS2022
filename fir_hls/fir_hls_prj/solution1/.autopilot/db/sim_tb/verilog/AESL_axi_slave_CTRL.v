...
//------------------------Parameter----------------------
`define TV_IN_len "./c.fir_wrap.autotvin_len.dat"
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_len [len_DEPTH - 1 : 0];
...
assign TRAN_CTRL_write_data_finish = 1 & len_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        if (write_one_len_data_done === 1) begin
            ...
            if (write_len_count == len_OPERATE_DEPTH) begin
                write_len_run_flag <= 0; 
            ...
            if (write_len_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (len_c_bitwidth < 32) begin
                        len_data_tmp_reg = mem_len[write_len_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < len_c_bitwidth) begin
                                len_data_tmp_reg[j] = mem_len[write_len_count][i*32 + j];
                            end
                            else begin
                    ...
                    write (len_data_in_addr + write_len_count * four_byte_num * 4 + i * 4, len_data_tmp_reg, write_len_resp);
                end
                process_busy = 0;
